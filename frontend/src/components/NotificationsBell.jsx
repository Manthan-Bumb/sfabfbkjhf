    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button data-testid="notif-bell" variant="ghost" size="icon" className="relative">
          ...
          {data.unread > 0 && (
            <span data-testid="notif-unread-count" className="absolute -top-0.5 -right-0.5 bg-orange-600 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
      ...
      <PopoverContent align="end" className="w-96 p-0 rounded-sm border-slate-200">
        <div className="flex items-center justify-between p-3 border-b border-slate-200">
          ...
          {data.unread > 0 && <button data-testid="notif-mark-all" onClick={markAll} className="text-xs text-orange-600 hover:underline">Mark all read</button>}
        </div>
        <div className="max-h-96 overflow-auto">
          {data.items.length === 0 && <div className="p-8 text-center text-sm text-slate-500">No notifications</div>}
          {data.items.map(n => (
            <div key={n.id} className={`p-3 border-b border-slate-100 ${!n.read ? "bg-orange-50/40" : ""}`} data-testid={`notif-item-${n.id}`}>
