...
client = AsyncMongoMockClient() if mongo_url.startswith("mongomock://") else AsyncIOMotorClient(mongo_url)
...
async def add_rate_card(data: RateCardIn, user=Depends(require_user)):
    ...
    existing = await db.rate_cards.find_one({
        "courier_id": user["id"],
        "pickup_city": {"$regex": f"^{re.escape(data.pickup_city)}$", "$options": "i"},
        "delivery_city": {"$regex": f"^{re.escape(data.delivery_city)}$", "$options": "i"},
        "transport_mode": data.transport_mode,
        "weight_slab": data.weight_slab,
    }, {"_id": 0})
    if existing:
        rc["id"] = existing["id"]
        rc["created_at"] = existing.get("created_at", now_iso())
        rc["updated_at"] = now_iso()
        await db.rate_cards.update_one({"id": existing["id"], "courier_id": user["id"]}, {"$set": rc})
    else:
...
async def seed_db():
    ...
    # Demo business account for local testing
    if not await db.users.find_one({"email": "business@logscanner.in"}):
        await db.users.insert_one({
            "id": str(uuid.uuid4()),
            "role": "business",
            "company_name": "Demo Manufacturing Pvt Ltd",
            "gst_number": "27ABCDE1234F1Z5",
            "contact_person": "Business User",
            "mobile": "9876543210",
            "email": "business@logscanner.in",
            "password": hash_password("business@123"),
            "mobile_verified": True,
            "email_verified": True,
            "gst_verified": True,
            "status": "active",
